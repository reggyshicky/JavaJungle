...
public class StudentManagementSystem extends Application {
    ...
    static class DataStore {
        ...
        final ObservableList<Enrollment> enrollments = FXCollections.observableArrayList();

        /* Seedingng some demo data so the app is not empty on first launch. */
        DataStore() {
            students.addAll(
    ...
    /* Returns a styled action button (blue). */
    ...
    /*  Returns a styled danger button (red). */
    ...
    /* Returns a styled secondary button (grey). */
     ...
     */
    static class StudentsPanel extends BorderPane {

        private final DataStore store;
        private final TableView<Student> table = new TableView<>();

        StudentsPanel(DataStore store) {
            this.store = store;
        ...
        private void openStudentDialog(Student existing) {
            ...
            if (existing != null) idField.setDisable(true); // ID not editable
     ...
     */
    static class CoursesPanel extends BorderPane {

        private final DataStore store;
        private final TableView<Course> table = new TableView<>();

        CoursesPanel(DataStore store) {
            this.store = store;
            setPadding(new Insets(20));
            setStyle("-fx-background-color:#F5F7FA;");
            ...
            deleteBtn.setOnAction(e -> {
                ...
                // Remove enrollments for this course too
        ...
        private void openCourseDialog() {
            ...
            saveBtn.setOnAction(e -> {
                ...
                boolean dup = store.courses.stream()
                    .anyMatch(c -> c.getCode().equalsIgnoreCase(code));
                if (dup) {
                    showError("Duplicate", "A course with code '" + code + "' already exists.");
                    return;
                }
     ...
     * The administrator selects a course from a dropdown, which then
     * displays students not yet enrolled. They pick a student and enroll.
     * A table below shows all current enrollments.
     */
    static class EnrollmentPanel extends BorderPane {

        private final DataStore store;

        EnrollmentPanel(DataStore store) {
            this.store = store;
            setPadding(new Insets(20));
            setStyle("-fx-background-color:#F5F7FA;");
            ...
            // When a course is selected, populate studentBox with eligible students
            // (those NOT already enrolled in this course)
            courseBox.setOnAction(e -> {
                ...
                studentBox.setPromptText(
                    eligible.isEmpty() ? "All students enrolled" : "Select a student…");
            });
            ...
            grCol.setCellValueFactory(new PropertyValueFactory<>("grade"));
            ...
            removeBtn.setOnAction(e -> {
                ...
                // Refresh eligible students if same course still selected
                courseBox.fireEvent(new javafx.event.ActionEvent());
            });

            VBox center = new VBox(12, title, form,
                                   new Separator(), tableTitle, table, removeBtn);
            setCenter(center);
            ...
            enrollBtn.setOnAction(e -> {
                ...
                if (store.isEnrolled(s, c)) {
                    showError("Already Enrolled",
                        s.getName() + " is already enrolled in " + c.getTitle() + ".");
                    return;
                }
     ...
     * The administrator picks a course and selects a grade from a
     * predefined list, then saves.
     */
    static class GradesPanel extends BorderPane {

        private final DataStore store;
        private final TableView<Enrollment> table = new TableView<>();

        GradesPanel(DataStore store) {
            this.store = store;
            setPadding(new Insets(20));
            setStyle("-fx-background-color:#F5F7FA;");
            ...
            // When student changes, populate courses they are enrolled in
            studentBox.setOnAction(e -> {
                ...
                enrollBox.setPromptText(
                    es.isEmpty() ? "Not enrolled in any course" : "Select course…");
            });

            // When enrollment selection changes, pre-fill current grade
            ...
            VBox center = new VBox(12, title, form,
                                   new Separator(), tableTitle, table);
            setCenter(center);
